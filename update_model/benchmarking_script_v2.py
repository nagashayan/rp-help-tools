...
# ==========================================
WINDOW_NAME = "Handshake Z-Vector Logic (MAC FINAL)"
WINDOW_WIDTH = 450
WINDOW_HEIGHT = 850

# Temporal Settings (RESTORED THE ARMOR)
WINDOW_SIZE = 10
THRESHOLD = 0.50
...
# Logic Thresholds (The Strict SBF Gates)
...
def init_mediapipe():
    ...
    options = vision.HandLandmarkerOptions(
        ...
        num_hands=3, # Keep at 3 for Mac, will drop to 1 for Pi later
...
print("==================================================")
print("🚀 STARTING FINAL MAC BENCHMARK (TEMPORAL AVERAGING ON)...")
print("==================================================")
...
for category in categories:
    ...
    for clip_name in clip_folders:
        ...
        # Reset the temporal queues for the new sequence
        coord_history.clear()
        prediction_queue.clear() # RESTORED: Clear the 10-frame queue
        sbf_latch_counter = 0 
        ...
        for img_path in image_paths:
            ...
            # --- FUSION & TEMPORAL AVERAGING (RESTORED) ---
            fused_pred = (0.4 * cnn_raw) + (0.4 * pose_score) + (0.2 * stability_score)
            
            # RESTORED: Add the instantaneous score to the 10-frame queue
            prediction_queue.append(fused_pred)
            
            # RESTORED: Calculate the rolling average
            ...
            clip_max_avg_conf = max(clip_max_avg_conf, avg_conf) # Now tracking the 10-frame average peak

            # RESTORED: Trigger based on the 10-frame average, not the instant frame
        ...
        if is_actual_handshake and system_triggered_handshake:
            ...
            false_positives += 1
            # --- NEW: THE FALSE POSITIVE AUTOPSY ---
            print(f"\n⚠️ FALSE POSITIVE DETECTED: {category}/{clip_name}")
            print(f"   -> Max CNN Raw Score : {clip_max_cnn:.3f}")
            print(f"   -> Max Fused Avg Conf: {clip_max_avg_conf:.3f} (Crossed {THRESHOLD})")
