...
def is_thumb_up(landmarks, handedness):
    ...
    # - Thumb_tip should be to the left (for right hand) or right (for left hand) of the
    # index finger
    if handedness == "Right":
        return (
            thumb_tip.y < thumb_mcp.y  # Thumb tip above MCP
            and thumb_tip.x < index_tip.x
        )  # Thumb tip left of index finger (for right hand)
    else:
        return (
            thumb_tip.y < thumb_mcp.y  # Thumb tip above MCP
            and thumb_tip.x > index_tip.x
        )  # Thumb tip right of index finger (for left hand)
